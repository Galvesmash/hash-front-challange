module.exports = {
  // TESTS FOR API WITH INTERNAL ERROR:
  'Request with internal error': browser => {
    browser.url('http://localhost:3000/?internalError').waitForElementVisible('body', 1000);
    // Fill inputs
    ...
    // Error Overlay
    browser.waitForElementVisible('#error-500', 5000);
    browser.moveTo(null, 0, 0).mouseButtonClick();
    browser.waitForElementNotVisible('#error-500', 5000);
    // Expected Results:
